// TODO:
// 1. Figure out why this took longer than the synchronous version...
...
if (isMainThread) {
	...
	for (let i = 0; i < numCPUs; i++) {
		...
		worker.postMessage({ passwords, hashes: childHashes });
	...
	parentPort.on('message', async ({ passwords, hashes }) => {
		...
		for (let hash of hashes) {
			...
			let queue = [findPlaintext({ passwordGetter }, hash, foundObj)];
